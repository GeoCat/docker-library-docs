...
-	[`12.3.1-ubi10-rc`, `12.3-ubi10-rc`, `12.3.1-ubi-rc`, `12.3-ubi-rc`](https://github.com/MariaDB/mariadb-docker/blob/769f60ce827e3465281d184080727f3f13f3f61e/12.3-ubi/Dockerfile)

-	[`12.3.1-noble-rc`, `12.3-noble-rc`, `12.3.1-rc`, `12.3-rc`](https://github.com/MariaDB/mariadb-docker/blob/a681c62cf8d84fc614e648776992733cdca60983/12.3/Dockerfile)

-	[`12.2.2-ubi10`, `12.2-ubi10`, `12-ubi10`, `12.2.2-ubi`, `12.2-ubi`, `12-ubi`](https://github.com/MariaDB/mariadb-docker/blob/604617ddd4d241538390b0bb97bf4cd92e5f4c6f/12.2-ubi/Dockerfile)

-	[`12.2.2-noble`, `12.2-noble`, `12-noble`, `noble`, `12.2.2`, `12.2`, `12`, `latest`](https://github.com/MariaDB/mariadb-docker/blob/604617ddd4d241538390b0bb97bf4cd92e5f4c6f/12.2/Dockerfile)

-	[`11.8.7-ubi9`, `11.8-ubi9`, `11-ubi9`, `lts-ubi9`, `11.8.7-ubi`, `11.8-ubi`, `11-ubi`, `lts-ubi`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/11.8-ubi/Dockerfile)

-	[`11.8.7-noble`, `11.8-noble`, `11-noble`, `lts-noble`, `11.8.7`, `11.8`, `11`, `lts`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/11.8/Dockerfile)

-	[`11.4.11-ubi9`, `11.4-ubi9`, `11.4.11-ubi`, `11.4-ubi`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/11.4-ubi/Dockerfile)

-	[`11.4.11-noble`, `11.4-noble`, `11.4.11`, `11.4`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/11.4/Dockerfile)

-	[`10.11.17-ubi9`, `10.11-ubi9`, `10-ubi9`, `10.11.17-ubi`, `10.11-ubi`, `10-ubi`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/10.11-ubi/Dockerfile)

-	[`10.11.17-jammy`, `10.11-jammy`, `10-jammy`, `10.11.17`, `10.11`, `10`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/10.11/Dockerfile)

-	[`10.6.26-ubi9`, `10.6-ubi9`, `10.6.26-ubi`, `10.6-ubi`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/10.6-ubi/Dockerfile)

-	[`10.6.26-jammy`, `10.6-jammy`, `10.6.26`, `10.6`](https://github.com/MariaDB/mariadb-docker/blob/cfc6a068e6d6418b22e3f0e498c243c2279a118a/10.6/Dockerfile)
...
The ppc64le/mariadb has a number of tags, and of note is `latest`, as the latest stable version, and `lts`, as the last long term support release.
...
```console
$ docker run --name some-mariadb -p 3306:3306 ppc64le/mariadb:latest
```
...
```console
$ docker run --detach --name some-mariadb --env MARIADB_ROOT_PASSWORD=my-secret-pw ppc64le/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_ALLOW_EMPTY_ROOT_PASSWORD=1 ppc64le/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_RANDOM_ROOT_PASSWORD=1 ppc64le/mariadb:latest
```
...
### Start a `ppc64le/mariadb` server instance with user, password and database

Starting a MariaDB instance with a user, password, and a database:

```console
$ docker run --detach --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_DATABASE=exmple-database --env MARIADB_ROOT_PASSWORD=my-secret-pw ppc64le/mariadb:latest
```

### Start a `ppc64le/mariadb` server instance in a network
...
```console
$ docker run -it --network some-network --rm ppc64le/mariadb mariadb -h some-mariadb -u example-user
```
...
```console
$ docker run -it --rm ppc64le/mariadb mariadb --host <server container IP> --user example-user --password --database test
```
...
The `docker exec` command allows you to run commands inside the running container. The following command line will give you a bash shell inside your `ppc64le/mariadb` container:
...
These disable the authentication of `user@hostname` users. To re-enable the `skip-name-resolve` use `disable-skip-name-resolve` as variable or argument. When enabled, the `host-cache-size` should be sufficient for the number of containers connecting to the `ppc64le/mariadb`.
...
When you start the `ppc64le/mariadb` image, you can adjust the initialization of the MariaDB instance by passing one or more environment variables on the `docker run` command line. Do note that all of the variables, except `MARIADB_AUTO_UPGRADE`, will have no effect if you start the container with a data directory that already contains a database. I.e. any pre-existing database will always be left untouched on container startup.
...
```console
$ docker run --name some-mysql -e MARIADB_ROOT_PASSWORD_FILE=/run/secrets/mariadb-root -d ppc64le/mariadb:latest
```
...
2.	Start your `ppc64le/mariadb` container like this:
	...
	```console
	$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql:Z -e MARIADB_ROOT_PASSWORD=my-secret-pw -d ppc64le/mariadb:latest
	```
...
Example compose files using this `ppc64le/mariadb` are located in https://github.com/MariaDB/mariadb-docker in the `/examples` folder.
