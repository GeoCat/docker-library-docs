...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `caddy` official image](https://hub.docker.com/_/caddy) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
$ docker run -d --cap-add=NET_ADMIN -p 80:80 -p 443:443 -p 443:443/udp \
    ...
    riscv64/caddy caddy file-server --domain example.com
```
...
Most users deploying production sites will not want to rely on mounting files into a container, but will instead base their own images on `riscv64/caddy`:

```Dockerfile
# note: never use the :latest tag in a production site
FROM riscv64/caddy:<version>
...
```Dockerfile
FROM riscv64/caddy:<version>-builder AS builder
...
Note the second `FROM` instruction - this produces a much smaller image by simply overlaying the newly-built binary on top of the regular `riscv64/caddy` image.
...
The `riscv64/caddy` images come in many flavors, each designed for a specific use case.

## `riscv64/caddy:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `riscv64/caddy:<version>-alpine`
