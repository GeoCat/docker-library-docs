...
3.	initialize the target directory separately from the final runtime (with a `chown` in between):
	...
	$ docker run -it --rm -v pg:/var/lib/postgresql -e POSTGRES_PASSWORD=mysecretpassword riscv64/postgres
	The files belonging to this database system will be owned by user "postgres".
	...
	( once it's finished initializing successfully and is waiting for connections, stop it )
	$ docker run -it --rm -v pg:/var/lib/postgresql bash chown -R 1000:1000 /var/lib/postgresql
	$ docker run -it --rm --user 1000:1000 -v pg:/var/lib/postgresql riscv64/postgres
...
2.	Start your `riscv64/postgres` container like this:
	...
	```console
	$ docker run --name some-postgres -v /my/own/datadir:/var/lib/postgresql -e POSTGRES_PASSWORD=mysecretpassword -d riscv64/postgres:tag
	```
...
The `riscv64/postgres` images come in many flavors, each designed for a specific use case.

## `riscv64/postgres:<version>`
...
## `riscv64/postgres:<version>-alpine`
