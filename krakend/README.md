...
**Note:** this is the "per-architecture" repository for the `amd64` builds of [the `krakend` official image](https://hub.docker.com/_/krakend) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
-	[`2.13.5`, `2.13`, `2`, `latest`](https://github.com/krakend/docker-library/blob/8c2abb57d14f937815fc3b87ef8c7d1e2cfee430/2.13.5/Dockerfile)
...
```bash
docker run -d -p 8080:8080 -v "$PWD:/etc/krakend/" amd64/krakend
...
```bash
docker run -p 8080:8080 -v "${PWD}:/etc/krakend/" amd64/krakend run -d -c /etc/krakend/krakend.json
```
...
```bash
docker run -it -v $PWD:/etc/krakend/ amd64/krakend check --config krakend.json
```

#### Show the help:

```bash
docker run --rm -it amd64/krakend help
```

### Building your custom KrakenD image

Most production deployments will not want to rely on mounting a volume for the container but to use their image based on `amd64/krakend`:

Your `Dockerfile` could look like this:

```Dockerfile
FROM amd64/krakend:<version>
...
```Dockerfile
FROM amd64/krakend:<version> as builder
...
```bash
docker run --rm -it amd64/krakend help
docker run --rm -it amd64/krakend krakend help
```
